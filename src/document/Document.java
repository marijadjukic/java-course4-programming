...
public abstract class Document {
	...
	protected int countSyllables(String word)
	{
		...
		int i = wordLower.length()-1;
		int eNumbers = 0;
		while (i >= 0 && wordLower.charAt(i) == 'e') {
			i--;
			eNumbers++;
		}
		int j = 0;
		int consonants = 0;
		while (j < wordLower.length()-eNumbers) {
			if (!isVowel(wordLower.charAt(j))) {
				consonants++;
			}
			j++;
		}
		if(consonants == wordLower.length()-eNumbers){
			count = 1;
		}
		while (i >= 0) {
	...
	//helper method checks if letter is vowel
	...
	public int getCountSyllables(String word){
	...
	public double getFleschScore()
	{
		...
		double fleshScore = 206.835d - 1.015d*(getNumWords())/(getNumSentences())
							-84.6d *(getNumSyllables())/(getNumWords());
	    return fleshScore;
	}
}
